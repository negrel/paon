...
func (hb *HBox) AppendChild(child pdkwidgets.Widget) error {
	...
	if err != nil {
		hb.flags = render.DrawFlag | render.LayoutFlag
	}
...
func (hb *HBox) Render(ctx render.Context) {
	needRedraw := pdkwidgets.IsDirty(hb.BaseLayout.BaseWidget) || ctx.Flags >= render.DrawFlag
	needReflow := !hb.cache.IsValid(ctx.Constraint) || ctx.Flags >= render.FullRenderFlag
