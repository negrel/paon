...
var keyPressType = pdkevents.NewType("KeyPress")

// KeyPressType returns the type for KeyPress events.
func KeyPressType() pdkevents.Type {
	return keyPressType
}

// KeyPressListener returns an events.Listener that will call the given handler
// on KeyPress events.
func KeyPressListener(handler func(KeyPress)) (pdkevents.Type, pdkevents.Handler) {
	return KeyPressType(), pdkevents.HandlerFunc(func(event pdkevents.Event) {
		assert.IsType(event, KeyPress{})
		handler(event.(KeyPress))
	})
}

// KeyPress define an event that is triggered when user press a key.
...
// NewKeyPress returns a new KeyPress event.
func NewKeyPress(modifiers ModMask, key Key, r rune) KeyPress {
	return KeyPress{
		Event:     pdkevents.NewEvent(keyPressType),
		Modifiers: modifiers,
		Key:       key,
		Rune:      r,
	}
